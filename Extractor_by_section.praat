form Save intervals to small WAV sound files
    comment What section and drill are you running this on?
    sentence section 1B
    sentence drill A
    ...
    sentence Suffix 
endform
filename$ = "JSL/" + section$ + "/" + section$ + "drill list.txt"
file$ < 'filename$'
    firstnewline = index (leftover$, newline$)
    drill$ = left$ (leftover$, (firstnewline - 1))
    leftoverlength = length (leftover$)
    leftover$ = right$ (leftover$, (leftoverlength - firstnewline))
#    sectiondrill$ = section$ + ".Dr." + drill$
    folder$ = "JSL/" + section$ + "/" + sectiondrill$ + "/"
    gridname$ = section$ + "_" + drill$
    soundname$ = section$ + "_" + drill$
